...
namespace ccsds {
   ...
   * are string, int, uint, float, bool, and byte vectors. Packet and Manager configuration
  ...
  public:
    ...
    using ConfigValue = std::variant<std::string, int, std::uint64_t, float, bool,
     ...
     */
    ResultBool load(const std::string &filename);
    ...
    Result<T> get(const std::string& key) const {
      ...
      RET_IF_ERR_MSG(it == values.end(), ErrorCode::NO_DATA,
  ...
  private:
     ...
     */
    static ResultBuffer parseBytes(const std::string &valueStr);
  };
} // namespace ccsds
