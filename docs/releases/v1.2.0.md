...
- Native Raspberry Pi 5 arm64 and STM32 Cortex-M7 hardware execution evidence.
...
This boundary is appropriate for a packet PDU library. Complete abstract services, lower-layer transfer, routing, and system-wide management belong to a larger flight or ground protocol entity when required.
...
The authoritative clause-level traceability is provided by [`CCSDS_COMPLIANCE.md`](../../CCSDS_COMPLIANCE.md).
...
### Automated and installed-package evidence
...
- Cortex-M7 consumer compilation and relocatable linking against `libccsdspack.a`.

### Native hardware evidence

- Raspberry Pi 5, native arm64 Debian 13.5, validated on 2026-07-26:
  - 93/93 installed tests;
  - CLI integration;
  - exact-version external CMake consumer;
  - `CCSDSPACK_AARCH64_TEST:PASS`.
- NUCLEO-H755ZI-Q Cortex-M7, validated on 2026-08-02 using the documented retained H745/H755 dual-core project mapping:
  - firmware build, link, flash, startup, and reset confirmed;
  - deterministic packet, CRC, Manager, Validator, PVN, and Idle tests passed;
  - `CCSDSPACK_MCU_TEST:PASS` reported twice over ST-LINK virtual COM at 115200 8N1.

See [`V1_2_HARDWARE_VALIDATION.md`](../V1_2_HARDWARE_VALIDATION.md) for the evidence boundary and reproduction procedures.
...
The packet implementation and hardware-validation gates are complete. Before publication, merge the validated `develop` state into `main`, confirm Linux and Windows CI at that exact commit, then create and verify the `v1.2.0` tag artifacts.

See `CHANGELOG.md` for the complete categorized change list, `CCSDS_COMPLIANCE.md` for clause-level traceability, and `docs/CCSDS_133_0_B_2_PROFILE.md` for the protocol profile.
